...
'''
crawler_iam_role = os.environ['crawler_iam_role']
tenant = os.environ['tenant']
env = os.environ['env']
TeamTag = os.environ['TeamTag']
...
def lambda_handler(event, context):
    ...
    try:
        ...
        key = event['detail']['resources'][0]['ARN'].split(':::',1)[1].split('/',1)[1]
        obj_list = event['detail']['resources'][0]['ARN'].split(':::',1)[1].split('/')
        
        s3_bucket = obj_list[0]
        vci_domain = obj_list[1]
        vci_table_name = obj_list[2]
